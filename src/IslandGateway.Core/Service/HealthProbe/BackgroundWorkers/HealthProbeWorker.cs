...
namespace IslandGateway.Core.Service.HealthProbe
{
    internal class HealthProbeWorker : IHealthProbeWorker
    {
        ...
        public HealthProbeWorker(ILogger<HealthProbeWorker> logger, IBackendManager backendManager, IBackendProberFactory backendProberFactory)
        {
            ...
            this._backendProberFactory = backendProberFactory;
        }
        ...
        public async Task UpdateTrackedBackends()
        {
            ...
            var proberAdded = 0;
            var backends = this._backendManager.GetItems();
            ...
            foreach (var backend in backends)
            {
                ...
                if (this._activeProbers.TryGetValue(backend.BackendId, out var activeProber))
                {
                        ...
                        activeProber.Config != backendConfig)
                    {
                        ...
                        this._logger.LogInformation($"Health check work stopping prober for '{backend.BackendId}'.");
                        stopTasks.Add(activeProber.StopAsync());
                        this._activeProbers.Remove(backend.BackendId);
                ...
                if (!createNewProber)
                {
                    ...
                    this._logger.LogInformation($"Health check prober for backend: '{backend.BackendId}' is disabled because {reason}.");
                }
                ...
                if (createNewProber)
                {
                    ...
                    var newProber = this._backendProberFactory.CreateBackendProber(backend.BackendId, backendConfig, backend.EndpointManager);
                    this._activeProbers.Add(backend.BackendId, newProber);
                    proberAdded++;
                    this._logger.LogInformation($"Prober for backend '{backend.BackendId}' has created.");
                    newProber.Start(this._semaphore);
                }
            }

            // Step 5: Stop and remove probes for backends that no longer exist.
            List<string> probersToRemove = new List<string>();
            foreach (var kvp in this._activeProbers)
            {
                if (!backendIdList.Contains(kvp.Key))
                {
                    // Gracefully shut down the probe.
                    this._logger.LogInformation($"Health check work stopping prober for '{kvp.Key}'.");
            ...
            probersToRemove.ForEach(p => this._activeProbers.Remove(p));
            await Task.WhenAll(stopTasks);
            this._logger.LogInformation($"Health check prober are updated. Added {proberAdded} probes, removed {probersToRemove.Count} probes. There are now {this._activeProbers.Count} probes.");
        }
        ...
        public async Task StopAsync()
        {
            ...
            await Task.WhenAll(stopTasks);
            this._logger.LogInformation($"Health check has gracefully shut down.");
        }
    }
}
