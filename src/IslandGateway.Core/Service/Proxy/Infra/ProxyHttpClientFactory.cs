...
namespace IslandGateway.Core.Service.Proxy.Infra
{
    ...
    internal class ProxyHttpClientFactory : IProxyHttpClientFactory
    {
        ...
        /// </summary>
        private readonly HttpMessageHandler normalHandler;
        ...
        {
            this.normalHandler = new SocketsHttpHandler
            {
                UseProxy = false,
        ...
        private HttpMessageHandler UpgradableHandler
        {
            get
            {
                ...
                var handler = Volatile.Read(ref this.upgradableHandler);
                if (handler == null)
                {
                    ...
                    if ((existingHandler = Interlocked.CompareExchange(ref this.upgradableHandler, handler, null)) != null)
                    {
                        handler.Dispose();
        ...
        public HttpMessageInvoker CreateNormalClient()
        {
            ...
            return new HttpMessageInvoker(this.normalHandler, disposeHandler: false);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!this.disposed)
            {
                if (disposing)
                {
                    ...
                    this.normalHandler.Dispose();

                    var currentUpgradableHandler = Volatile.Read(ref this.upgradableHandler);
                    if (currentUpgradableHandler != null)
                    {
                        if (Interlocked.CompareExchange(ref this.upgradableHandler, null, currentUpgradableHandler) == currentUpgradableHandler)
                        {
                        }
                    }
                }
