...
1. `Cookie` - stores the key as a cookie. It expects the request's key to be delivered as a cookie with configured name and sets the same cookie with `Set-Cookie` header on the first response in an affinitized sequence. This mode is implemented by `CookieSessionAffinityProvider`. Cookie name can be set via `SessionAffinityConfig.AffinityKeyName` which is used by `CookieSessionAffinityProvider` for this purpose. The default name is `.Yarp.ReverseProxy.Affinity`. Other cookie's properties can be configured via `SessionAffinityCookieConfig`.

2. `CustomHeader` - stores the key on a header. It expects the affinity key to be delivered in a customer header with configured name and sets the same header on the first response in an affinitized sequence. This mode is implemented by `CustomHeaderSessionAffinityProvider`. The header name can be set via `SessionAffinityConfig.AffinityKeyName` which is used by `CustomHeaderSessionAffinityProvider` for this purpose. The default name is `X-Yarp-Proxy-Affinity`.
