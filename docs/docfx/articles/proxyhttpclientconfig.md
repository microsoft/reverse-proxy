...
The following is an example of `ProxyHttpClientOptions` usage based on a code-first configuration sample project `ReverseProxy.Code.Sample`. An instance of `ProxyHttpClientOptions` is assigned to the [Cluster.HttpClientOptions](xref:Microsoft.ReverseProxy.Abstractions.Cluster.HttpClientOptions) property before passing the `Cluster` array to `LoadFromMemory` method.
