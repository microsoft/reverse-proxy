...
Query Parameter are specified in the `Match` section of a proxy route.
...
The query parameter name to check for on the request. A non-empty value is required. This field is case-insensitive per the HTTP RFCs.

### Values

A list of possible values to search for. The query parameter must match at least one of these values according to the specified `Mode`. At least one value is required unless `Mode` is set to `Exists`.
...
- Contains - The query parameter must exist and contains the non-empty value.
- NotContains - The query parameter must exist and not contain the non-empty value.
...
A query parameter with multiple values is not currently supported and will _not_ match.
```
QueryParam1: Value1, Value2
```
